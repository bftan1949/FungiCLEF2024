...
def run(gpu, cfg):
    ...
    close_optimizer = getattr(optim, cfg.TRAIN.CLOSESET.OPTIMIZER.NAME)(
        ddp_model.parameters(), **cfg.TRAIN.CLOSESET.OPTIMIZER.PARAMS
    )
    ...
    if 'convnext' in cfg.BASIC.MODEL.NAME:
        ...
        open_optimizer = getattr(optim, cfg.TRAIN.OPENSET.OPTIMIZER.NAME)(
            dummy_head.parameters(), **cfg.TRAIN.OPENSET.OPTIMIZER.PARAMS
        )
        ...
        open_optimizer = getattr(optim, cfg.TRAIN.OPENSET.OPTIMIZER.NAME)(
            list(dummy_head.parameters()) + list(dummy_aux_head.parameters()), **cfg.TRAIN.OPENSET.OPTIMIZER.PARAMS
        )
    else:
        original_head = ddp_model.module.head
        dummy_head = get_openset_head(original_head, cfg.TRAIN.OPENSET.C, gpu)
    ...
    ########################################################
    if cfg.TRAIN.CLOSESET.SCHEDULER.NAME == 'WarmUpScheduler':
        main_scheduler = lr_scheduler.CosineAnnealingLR(
            close_optimizer, **cfg.TRAIN.CLOSESET.SCHEDULER.COSINE_PARAMS
        )
        close_scheduler = WarmUpScheduler(
            close_optimizer,
            main_scheduler,
            len_loader=len(train_loader),
            warmup_steps=len(train_loader),
            **cfg.TRAIN.CLOSESET.SCHEDULER.WARMUP_PARAMS
        )
    else:
        close_scheduler = getattr(lr_scheduler, cfg.TRAIN.CLOSESET.SCHEDULER.NAME)(
            optimizer=close_optimizer, **cfg.TRAIN.CLOSESET.SCHEDULER.PARAMS
        )
    ...
    if type(cfg.TRAIN.CLOSESET.LOSS.NAME) == list:
        cls_loss_fn = getattr(loss, cfg.TRAIN.CLOSESET.LOSS.NAME[0])().cuda(gpu)
        cost_loss_fn = getattr(loss, cfg.TRAIN.CLOSESET.LOSS.NAME[1])().cuda(gpu)
    else:
        cls_loss_fn = getattr(loss, cfg.TRAIN.CLOSESET.LOSS.NAME)().cuda(gpu)
        cost_loss_fn = None
    ...
    #################### train closeset ########################
    # for epoch in range(start_epoch, cfg.TRAIN.CLOSESET.EPOCHS):
    #     train_loader.sampler.set_epoch(epoch)
    #
    #     train_one_epoch(ddp_model, close_optimizer, train_loader, cls_loss_fn,
    #                     cost_loss_fn, epoch, gpu, scaler, logger, cfg)
    #
    #     close_scheduler.step()
    #
    ...
    #             }
    #             checkpoint_path = checkpoint_folder / f'closeset_checkpoint.best.pt'
