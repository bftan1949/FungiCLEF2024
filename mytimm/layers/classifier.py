...
class NormDynamicMlpClassifierHead(nn.Module):
    ...
    def __init__(
        ...
        self.loc_att = get_dynamic_mlp(hidden_size, mlp_d, mlp_h, mlp_n, mlp_type)
    ...
    def forward(self, x, loc, pre_logits: bool = False):
        x = self.global_pool(x)
        x = self.norm(x)
        x = self.flatten(x)
        x = self.pre_logits(x)
