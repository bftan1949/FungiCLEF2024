...
from mytimm.layers.dynamic_mlp import FCNet, get_dynamic_mlp
try:
...
from mytimm.layers import PatchEmbed, Mlp, DropPath, AttentionPoolLatent, RmsNorm, PatchDropout, SwiGLUPacked, \
...
class VisionTransformer(nn.Module):
    ...
    def _pos_embed(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.no_embed_class:
            ...
            # original mytimm, JAX, and deit vit impl
...
class ViTDynamicMLP(nn.Module):
    ...
    def __init__(
        ...
        self.loc_net = FCNet(num_inputs=mlp_cin, num_classes=mlp_d, num_filts=256)
        self.loc_att = get_dynamic_mlp(self.embed_dim, mlp_d, mlp_h, mlp_n, mlp_type)
    ...
    def forward_head(self, x: torch.Tensor, loc, pre_logits: bool = False) -> torch.Tensor:
        if self.attn_pool is not None:
            x = self.attn_pool(x)
        elif self.global_pool == 'avg':
            x = x[:, self.num_prefix_tokens:].mean(dim=1)
    ...
    def forward(self, x: torch.Tensor, loc) -> torch.Tensor:
        x = self.forward_features(x)
        x = self.forward_head(x, loc)
        return x


def init_weights_vit_timm(module: nn.Module, name: str = '') -> None:
    """ ViT weight initialization, original mytimm impl (for reproducibility) """
...
def _convert_dinov2(
    ...
    if 'register_tokens' in state_dict:
        # convert dinov2 w/ registers to no_embed_class mytimm model (neither cls or reg tokens overlap pos embed)
...
def checkpoint_filter_fn(
    ...
    if 'visual.class_embedding' in state_dict:
        ...
        # OpenCLIP model with mytimm vision encoder
        # FIXME remap final nn.Linear if it exists outside of the mytimm .trunk (ie in visual.head.proj)
        prefix = 'visual.trunk.'
...
default_cfgs = {
    ...
    'vit_base_patch32_clip_224.openai_ft_in12k_in1k': _cfg(
        # hf_hub_id='mytimm/vit_base_patch32_clip_224.openai_ft_in12k_in1k',  # FIXME weight exists, need to push
        mean=OPENAI_CLIP_MEAN, std=OPENAI_CLIP_STD),
    ...
    'vit_base_patch32_clip_224.laion2b_ft_in12k': _cfg(
        # hf_hub_id='mytimm/vit_base_patch32_clip_224.laion2b_ft_in12k',  # FIXME weight exists, need to push
    ...
    'vit_base_patch32_clip_224.openai_ft_in12k': _cfg(
        # hf_hub_id='mytimm/vit_base_patch32_clip_224.openai_ft_in12k',  # FIXME weight exists, need to push
        mean=OPENAI_CLIP_MEAN, std=OPENAI_CLIP_STD, num_classes=11821),
    ...
    'vit_base_patch32_clip_224.openai': _cfg(
        hf_hub_id='mytimm/vit_base_patch32_clip_224.openai',
        notes=('natively QuickGELU, use quickgelu model variant for original results',),
        mean=OPENAI_CLIP_MEAN, std=OPENAI_CLIP_STD, num_classes=512),
    'vit_base_patch16_clip_224.openai': _cfg(
        hf_hub_id='mytimm/vit_base_patch16_clip_224.openai',
        notes=('natively QuickGELU, use quickgelu model variant for original results',),
        mean=OPENAI_CLIP_MEAN, std=OPENAI_CLIP_STD, num_classes=512),
    'vit_large_patch14_clip_224.openai': _cfg(
        hf_hub_id='mytimm/vit_large_patch14_clip_224.openai',
        notes=('natively QuickGELU, use quickgelu model variant for original results',),
        mean=OPENAI_CLIP_MEAN, std=OPENAI_CLIP_STD, crop_pct=1.0, num_classes=768),
    'vit_large_patch14_clip_336.openai': _cfg(
        hf_hub_id='mytimm/vit_large_patch14_clip_336.openai', hf_hub_filename='open_clip_pytorch_model.bin',
        notes=('natively QuickGELU, use quickgelu model variant for original results',),
        mean=OPENAI_CLIP_MEAN, std=OPENAI_CLIP_STD,
    ...
    'vit_base_patch16_siglip_224.webli': _cfg(
        hf_hub_id='mytimm/ViT-B-16-SigLIP',
        hf_hub_filename='open_clip_pytorch_model.bin',
        num_classes=0),
    'vit_base_patch16_siglip_256.webli': _cfg(
        hf_hub_id='mytimm/ViT-B-16-SigLIP-256',
        hf_hub_filename='open_clip_pytorch_model.bin',
        input_size=(3, 256, 256),
        num_classes=0),
    'vit_base_patch16_siglip_384.webli': _cfg(
        hf_hub_id='mytimm/ViT-B-16-SigLIP-384',
        hf_hub_filename='open_clip_pytorch_model.bin',
        input_size=(3, 384, 384),
        num_classes=0),
    'vit_base_patch16_siglip_512.webli': _cfg(
        hf_hub_id='mytimm/ViT-B-16-SigLIP-512',
        hf_hub_filename='open_clip_pytorch_model.bin',
        input_size=(3, 512, 512),
        num_classes=0),
    'vit_large_patch16_siglip_256.webli': _cfg(
        hf_hub_id='mytimm/ViT-L-16-SigLIP-256',
        hf_hub_filename='open_clip_pytorch_model.bin',
        input_size=(3, 256, 256),
        num_classes=0),
    'vit_large_patch16_siglip_384.webli': _cfg(
        hf_hub_id='mytimm/ViT-L-16-SigLIP-384',
        hf_hub_filename='open_clip_pytorch_model.bin',
        input_size=(3, 384, 384),
        num_classes=0),
    'vit_so400m_patch14_siglip_224.webli': _cfg(
        hf_hub_id='mytimm/ViT-SO400M-14-SigLIP',
        hf_hub_filename='open_clip_pytorch_model.bin',
        num_classes=0),
    'vit_so400m_patch14_siglip_384.webli': _cfg(
        hf_hub_id='mytimm/ViT-SO400M-14-SigLIP-384',
        hf_hub_filename='open_clip_pytorch_model.bin',
        input_size=(3, 384, 384),
        num_classes=0),
...
def _create_vision_transformer(variant: str, pretrained: bool = False, model_cls=None,
...
@register_model
def vit_large_dynamic_mlp_patch14_clip_336(pretrained: bool = False, **kwargs) -> VisionTransformer:
    """ ViT-Large model (ViT-L/14) CLIP image tower @ 336x336
    """
    model_args = dict(patch_size=14, embed_dim=1024, depth=24, num_heads=16, pre_norm=True, norm_layer=nn.LayerNorm)
    model = _create_vision_transformer(
        'vit_large_patch14_clip_336', model_cls=ViTDynamicMLP, pretrained=pretrained, **dict(model_args, **kwargs))
    return model
...
def vit_large_patch14_clip_quickgelu_224(pretrained: bool = False, **kwargs) -> VisionTransformer:
    ...
    """
    from mytimm.layers import get_act_layer
    model_args = dict(
