...
Model defs atto, femto, pico, nano and _ols / _hnf variants are mytimm originals.
...
from mytimm.layers import trunc_normal_, AvgPool2dSame, DropPath, Mlp, GlobalResponseNormMlp, \
    LayerNorm2d, LayerNorm, create_conv2d, get_act_layer, make_divisible, to_ntuple
from mytimm.layers import NormMlpClassifierHead, ClassifierHead
from mytimm.layers.classifier import NormDynamicMlpClassifierHead
...
class ConvNeXtDynamicMLP(nn.Module):
    r""" ConvNeXt
        A PyTorch impl of : `A ConvNet for the 2020s`  - https://arxiv.org/pdf/2201.03545.pdf
    """
    ...
    def __init__(
        ...
        if head_norm_first:
            ...
            self.norm_pre = nn.Identity()
            self.head = NormDynamicMlpClassifierHead(
                self.num_features,
                num_classes,
                hidden_size=head_hidden_size,
                pool_type=global_pool,
    ...
    def forward_head(self, x, loc, pre_logits: bool = False):
        return self.head(x, loc, pre_logits=True) if pre_logits else self.head(x, loc)

    def forward(self, x, loc):
        x = self.forward_features(x)  # Nx1536x12x12
...
def _create_convnext(variant, pretrained=False, model_cls=None, **kwargs):
    if not model_cls:
        model_cls = ConvNeXt
...
def convnext_atto(pretrained=False, **kwargs) -> ConvNeXt:
    # mytimm femto variant (NOTE: still tweaking depths, will vary between 3-4M param, current is 3.7M
...
def convnext_atto_ols(pretrained=False, **kwargs) -> ConvNeXt:
    # mytimm femto variant with overlapping 3x3 conv stem, wider than non-ols femto above, current param count 3.7M
...
def convnext_pico_ols(pretrained=False, **kwargs) -> ConvNeXt:
    # mytimm nano variant with overlapping 3x3 conv stem
...
def convnext_nano(pretrained=False, **kwargs) -> ConvNeXt:
    # mytimm nano variant with standard stem and head
...
@register_model
def convnext_large_dynamic_mlp(pretrained=False, **kwargs) -> ConvNeXt:
    model_args = dict(depths=[3, 3, 27, 3], dims=[192, 384, 768, 1536], head_hidden_size=1536)
    model = _create_convnext('convnext_large_mlp', model_cls=ConvNeXtDynamicMLP, pretrained=pretrained,
