...
from mytimm.layers import Mlp, ConvMlp, DropPath, LayerNorm, ClassifierHead, NormMlpClassifierHead
from mytimm.layers import create_attn, get_act_layer, get_norm_layer, get_norm_act_layer, create_conv2d, create_pool2d
from mytimm.layers import trunc_normal_tf_, to_2tuple, extend_tuple, make_divisible, _assert
from mytimm.layers import RelPosMlp, RelPosBias, RelPosBiasTf, use_fused_attn, resize_rel_pos_bias_table
...
def _rw_coat_cfg(
    ...
    # 'RW' mytimm variant models were created and trained before seeing https://github.com/google-research/maxvit
    # Common differences for initial mytimm models:
    ...
    # - mbconv uses silu in mytimm, not gelu
...
def _rw_max_cfg(
    ...
    # 'RW' mytimm variant models were created and trained before seeing https://github.com/google-research/maxvit
    # Differences of initial mytimm models:
    # - mbconv expansion calculated from input instead of output chs
    # - mbconv shortcut and final 1x1 conv did not have a bias
    # - mbconv uses silu in mytimm, not gelu
    # - expansion in attention block done via output proj, not input proj
...
model_cfgs = dict(
    # mytimm specific CoAtNet configs
...
default_cfgs = generate_default_cfgs({
    # mytimm specific CoAtNet configs, ImageNet-1k pretrain, fixed rel-pos
    ...
    # mytimm specific CoAtNet configs, ImageNet-12k pretrain w/ 1k fine-tune, fixed rel-pos
    ...
    # mytimm specific MaxVit configs, ImageNet-1k pretrain or untrained
    ...
    # mytimm specific MaxVit w/ MLP rel-pos, ImageNet-1k pretrain
    ...
    # mytimm specific MaxVit w/ ImageNet-12k pretrain and 1k fine-tune
    ...
    # mytimm specific MaxVit w/ ImageNet-12k pretrain
    'maxvit_rmlp_base_rw_224.sw_in12k': _cfg(
        hf_hub_id='mytimm/',
        num_classes=11821,
    ),

    # mytimm MaxxViT configs (ConvNeXt conv blocks mixed with MaxVit transformer blocks)
    ...
    # mytimm MaxxViT-V2 configs (ConvNeXt conv blocks mixed with MaxVit transformer blocks, more width, no block attn)
